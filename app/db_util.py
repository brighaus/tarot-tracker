...
class DBUtil:
    ...
    def format_daily_data(self, data):
        #get data ready for loading into dict
        final_data = data
        return final_data

    def enter_daily(self):
        # command line support for entering a daily entry
        daily_fields = {'reversed': 't|f',
                        'date': 'm/d/yyyy',
                        'suit': 'cups, wands, swords, pentacles, trumps',
                        'rank': '1 - 22'}
        ic = Inputclient(daily_fields)
        #new_entry_data = ic.get_terminal_input()
        new_entry_data = ic.get_gui_input()

        new_entry = self.format_daily_data(new_entry_data)
        pprint.pprint(new_entry)
        if self.confirm_update() is True:
            return self.new_member(new_entry)
        else:
            return None
...
def main(argv):
    ...
    parser.add_argument('-t', '--test', help='execute self test',
                        action='store_true')
    parser.add_argument('-n', '--new', help='enter new daily tt record',
                        action='store_true')
    parser.add_argument('-u', '--update', help='update existing daily tt record',
                        action='store_true')
